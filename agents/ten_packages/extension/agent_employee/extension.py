...
class OpenAIV2VExtension(Extension):
    ...
    def on_audio_frame(self, ten_env: TenEnv, audio_frame: AudioFrame) -> None:
        try:
            ...
            asyncio.run_coroutine_threadsafe(self._on_audio(frame_buf), self.loop)
        except:
                    ...
                    match message:
                        case SessionCreated():
                            ...
                            if self.retrieved:
                                ...
                                logger.info(
                                    f"after append retrieve: {len(self.retrieved)}"
                                )
                        ...
                        case ItemInputAudioTranscriptionCompleted():
                            logger.info(f"On request transcript {message.transcript}")
                        ...
                        case ItemCreated():
                            ...
                            if self.max_history and (
                                "status" not in message.item
                                or message.item["status"] == "completed"
                            ):
                                # need maintain the history
                                await self._append_history(message.item)
                        case ResponseCreated():
                            response_id = message.response.id
                            logger.info(f"On response created {response_id}")
                        case ResponseDone():
                            id = message.response.id
                            status = message.response.status
                            logger.info(f"On response done {id} {status}")
                        ...
                        case ResponseAudioTranscriptDone():
                            ...
                            self.transcript = ""
                            self._send_transcript(ten_env, "", Role.Assistant, True)
                        ...
                        case ResponseFunctionCallArgumentsDone():
                            ...
                            await self.registry.on_func_call(
                                tool_call_id, name, arguments, self._on_tool_output
                            )
                        case ErrorMessage():
    ...
    def _fetch_properties(self, ten_env: TenEnv):
        ...
        try:
            ...
            logger.info(f"GetProperty optional {PROPERTY_AUDIO_OUT} failed, err: {err}")
        ...
        try:
            ...
            logger.info(f"GetProperty optional {PROPERTY_GREETING} error: {err}")
        ...
        try:
            ...
            logger.info(f"GetProperty optional {PROPERTY_DUMP} error: {err}")
                ...
                if r["role"] == MessageRole.User:
                    await self.conn.send_request(
                        ItemCreate(
                            item=UserMessageItemParam(
                                content=[
                                    {"type": ContentType.InputText, "text": r["input"]}
                                ]
                            )
                        )
                    )
    ...
    def _on_audio_delta(self, ten_env: TenEnv, delta: bytes) -> None:
        ...
        logger.debug(
            "on_audio_delta audio_data len {} samples {}".format(
    ...
    def _append_context(
        self, ten_env: TenEnv, sentence: str, stream_id: int, role: str
    ):
        ...
        try:
            ...
            logger.info(
                f"append_contexttext [{sentence}] stream_id {stream_id} role {role}"
            )
            ten_env.send_data(d)
        except:
    ...
    def _send_transcript(
        self, ten_env: TenEnv, content: str, role: Role, is_final: bool
    ) -> None:
        ...
        def send_data(
            ten_env: TenEnv, sentence: str, stream_id: int, role: str, is_final: bool
        ):
            try:
                ...
                logger.exception(f"Error send text data {role}: {sentence} {is_final}")
    ...
    def _on_tool_register(self, ten_env: TenEnv, cmd: Cmd):
        try:
            ...
            self.registry.register(
                name=name, description=description, callback=p, parameters=parameters
            )
    ...
    async def _remote_tool_call(
        self, ten_env: TenEnv, name: str, args: str, callback: Awaitable
    ):
    ...
    def save_to_file(
        self, user_message: str, role: str, file_name="employee_conversation.txt"
    ):
